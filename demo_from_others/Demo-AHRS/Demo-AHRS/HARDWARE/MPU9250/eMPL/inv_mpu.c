...
#define i2c_write   Multi_Write
#define i2c_read    Multi_Read
...
//如果定义了AK89XX磁力计，则定义以下寄存器
#ifdef AK89xx_SECONDARY
#define AKM_REG_WHOAMI      (0x00)

#define AKM_REG_ST1         (0x02)
#define AKM_REG_HXL         (0x03)//03-08磁力计数据寄存器 X-LH   Y-LH  Z-LH
...
//如果定义的是MPU6500(MPU9250为MPU6500+AKM8963)
#elif defined MPU6500
const struct gyro_reg_s reg = {
    .who_am_i       = 0x75,
    .rate_div       = 0x19,
...
{
    unsigned char tmp;
	printf("st.chip_cfg.dmp_on = %d\n",st.chip_cfg.dmp_on);
    if (st.chip_cfg.dmp_on)// st.chip_cfg.dmp_on = 0 表示已经开启了DMP
	{
        if (enable)
            tmp = BIT_DMP_INT_EN;
        else
	...
	{
		//printf("st.chip_cfg.int_enable = %d\n",st.chip_cfg.int_enable);
		if (!st.chip_cfg.sensors)//st.chip_cfg.sensors = 0xFF
            return -1;
...
int mpu_init(void)
{
    ...
    data[0] = BIT_RESET;//往PWR_MGMT_1写入0x80，最高位写1 复位MPU9250  MPU9250进入休眠模式
    ...
    data[0] = 0x00;//往PWR_MGMT_1写入0x00，唤醒MPU9250
    if (i2c_write(st.hw->addr, st.reg->pwr_mgmt_1, 1, &(data[0])))
        return -1;
    ...
    st.chip_cfg.clk_src = 0x01;//0x01 INV_CLK_PLL
    ...
    if (mpu_set_gyro_fsr(2000))//设置陀螺仪满量程为±2000dps
        return -1;
    if (mpu_set_accel_fsr(2))//设置就速度计满量程为±2g
        return -1;
    if (mpu_set_lpf(42))//设置数字低通滤波器滤波带宽为42HZ
        return -1;
    if (mpu_set_sample_rate(50))//设置采样率为50HZ
        return -1;
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    sensors &= ~INV_XYZ_COMPASS;//屏蔽最低位 即罗盘数据不能进入FIDO

    if (st.chip_cfg.dmp_on)//如果DMP打开了
	{
        return 0;
	}
    else 
	{
        ...
        prev = st.chip_cfg.fifo_enable;//prev = 0xFF
		//printf("prev = 0x%02x\n",prev);
        st.chip_cfg.fifo_enable = sensors & st.chip_cfg.sensors;//0x78
		//printf("fifo = 0x%02x\n", st.chip_cfg.fifo_enable);
		//printf("sensors = 0x%02x\n",sensors);
        if (st.chip_cfg.fifo_enable != sensors)//sensor = 0x78
            ...
            result = 0;
        if (sensors || st.chip_cfg.lp_accel_mode)//0x78 || 0x00  
		{
            //printf("here\n");
			set_int_enable(1);//开中断
        }
		else
            set_int_enable(0);
        if (sensors) 
...
int mpu_set_sensors(unsigned char sensors)
{
	...
	{
        data = 0x01;//INV_CLK_PLL=0x01  设置CLKSEL,PLL X轴为参考
		//printf("aaaaa\n");
	}
    else if (sensors)
        data = 0;
...
#ifdef AK89xx_SECONDARY
#ifdef AK89xx_BYPASS
    if (sensors & INV_XYZ_COMPASS)//如果开启了罗盘  则使能旁路模式
        mpu_set_bypass(1);
    else
        mpu_set_bypass(0);
#else
 ...
 *  This function should be used if the packet is to be parsed elsewhere.
	如果该数据包将被解析到其它地方，则应该使用此函数
 *  @param[in]  length  Length of one FIFO packet.	FIFO包长度
 *  @param[in]  data    FIFO packet.				FIFO包
 *  @param[in]  more    Number of remaining packets.剩余包数量
 */
    ...
    unsigned char *more)
{
    ...
    fifo_count = (tmp[0] << 8) | tmp[1];//获取FIFO中的数据长度(字节为单位)
    ...
    if (fifo_count > (st.hw->max_fifo >> 1)) 
	{
        ...
        if (tmp[0] & BIT_FIFO_OVERFLOW) //0x10  FIFO溢出中断
		{
            mpu_reset_fifo();
            return -2;
        }
